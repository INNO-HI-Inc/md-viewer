...
function sanitizeElement(el) {
    ...
    Array.from(el.attributes).forEach(function (attr) {
        if (!isAllowedAttribute(tag, attr.name, attr.value, el)) {
...
function isAllowedAttribute(tag, name, value, el) {
    ...
    if (attr === 'href' && tag === 'a') return isSafeUrl(value, false);
    if (attr === 'src' && tag === 'img') return isSafeUrl(value, true);
    if ((attr === 'alt' || attr === 'loading') && tag === 'img') return true;
    if ((attr === 'width' || attr === 'height') && tag === 'img') return /^\d{1,4}%?$/.test(value);
    if (attr === 'data-math' && tag === 'div') return /\bmath-block\b/.test(el.getAttribute('class') || '');
    return false;
}
