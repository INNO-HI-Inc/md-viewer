...
PDF 변환을 다양한 상황에서 검증(넓은 표·긴 코드·수식·이미지·이모지 등)하고 고쳤습니다.

### Fixed

- **넓은 표가 오른쪽에서 잘리던 문제** — 컬럼이 많은 표(월별 실적표 등)가 페이지 폭을 넘어 오른쪽이 잘렸습니다. 이제 페이지에 맞게 글자 크기를 자동으로 줄여 **표 전체가 잘리지 않고** 들어갑니다.
- **긴 코드 줄이 오른쪽에서 잘리던 문제** — 줄바꿈 없는 긴 코드 한 줄이 잘렸습니다. 이제 코드가 **줄바꿈되어** 전부 보입니다.
- **base64(data:) 이미지가 아예 안 나오던 문제** — 마크다운에 포함된 `data:image/...` 이미지가 프리뷰·PDF 양쪽에서 통째로 사라졌습니다(보안 필터가 과하게 차단). 이제 이미지에 한해 허용합니다(`javascript:`·`data:` 링크는 계속 차단, `<img>` 안의 SVG는 스크립트가 실행되지 않아 안전).

### Known issue

- 수식의 **제곱근(√)** 기호가 PDF에서 윗줄만 나오고 갈고리가 빠질 수 있습니다(html2canvas가 KaTeX의 SVG 슬라이스를 제대로 못 그림). 분수·지수·적분 등 다른 수식은 정상입니다. 다음 업데이트에서 개선 예정입니다.

## [1.0.41] - 2026-08-04

### Changed

- **Mermaid 기본값을 흑백으로 변경** — 요청에 따라 다이어그램은 이제 기본이 흑백입니다. 색을 넣고 싶으면 다이어그램 오른쪽 위 ◐ 버튼을 누르면 됩니다(문서별로 저장). 항상 색으로 보고 싶으면 설정 `mdPrettyViewer.mermaidLevelColors`를 켜세요.
- **PDF에서 큰 다이어그램을 더 크게** — 한 페이지에 통째로 담되(잘림 없음) 페이지의 최대 80%까지 채워 더 크고 읽기 좋게 표시합니다.

### Note

- 예전 버전(v1.0.37 무렵)으로 내보낸 PDF는 다이어그램이 페이지 경계에서 **쪼개져 잘릴 수** 있습니다. 현재 버전은 항상 **통째로** 담으니, VS Code를 재시작한 뒤 다시 내보내면 됩니다.

## [1.0.40] - 2026-08-04

### Added

- **시퀀스 다이어그램도 색상 적용** — 참여자(액터)마다 서로 다른 색으로 칠해져 레인을 구분하기 쉽습니다. (위·아래 박스 동일 색, 라이브·PDF 공통)

### Changed

- **색상 켜기/끄기(흑백) 버튼을 다이어그램 위로 이동** — 상단 툴바가 아니라, 다이어그램에 마우스를 올리면 오른쪽 위 확대(⤢) 버튼 옆에 흑백 토글 버튼(◐)이 뜹니다. 눌러서 색상 ↔ 흑백을 바로 전환할 수 있고 선택은 저장됩니다. (설정 `mdPrettyViewer.mermaidLevelColors`로 기본값 지정은 그대로)

## [1.0.39] - 2026-08-04

### Fixed

- **시퀀스 다이어그램 아래쪽이 "끝이 없어" 보이던 문제** — 아래쪽 액터(참여자) 박스가 빠져 수명선이 허공에서 끊긴 것처럼 보였습니다. 이제 위·아래 모두 액터 박스가 표시되어 다이어그램이 깔끔하게 닫힙니다. (라이브·PDF 공통)
- **복잡한(긴) 플로우차트가 프리뷰 아래로 넘쳐 잘려 보이던 문제** — 화면보다 긴 다이어그램이 아래로 흘러 잘린 것처럼 보였습니다. 이제 프리뷰 안에서 **화면 높이에 맞게 통째로 축소**해 전체가 한눈에 보이고, 자세히 보려면 ⤢(확대) 버튼으로 확대·이동하면 됩니다.

### Added

- **Mermaid 계층별 색상 켜기/끄기** — 다이어그램이 있는 문서에서 상단 툴바에 색상 토글 버튼(●●●●)이 나타납니다. 눌러서 계층별 색상 ↔ 단색을 바로 전환할 수 있고, 선택은 저장됩니다. 기본값은 설정 `mdPrettyViewer.mermaidLevelColors`(기본 켜짐)로도 지정할 수 있습니다.

## [1.0.38] - 2026-08-04

Mermaid 대규모 개선 — 많은 분들이 쓰시는 기능이라 PDF·색상·크기를 전면 손봤습니다.

### Fixed

- **🚨 PDF에서 큰 다이어그램이 잘리거나(밑부분 실종) 통째로 빠지던 문제** — 한 페이지보다 큰 플로우차트가 페이지 경계에서 잘리고, 시퀀스 다이어그램이 아예 사라지곤 했습니다. 이제 각 다이어그램을 **한 페이지에 들어가도록 자동 축소해 통째로** 담습니다. 절대 잘리지 않습니다.
- **PDF 플로우차트 아래 거대한 빈 공간(빈 페이지)** — 다이어그램이 제목과 같은 페이지에 못 들어가고 다음 장으로 밀리며 앞 페이지가 통째로 비던 문제. 이제 제목·본문과 같은 페이지에 배치됩니다.
- **시퀀스 다이어그램이 오른쪽/아래가 잘리던 문제** — 카드 폭에 맞춰 비율을 유지하도록 고쳐, 액터·메시지·수명선 끝까지 온전히 나옵니다.
- **다이어그램 글씨가 아래쪽에서 살짝 잘리던 문제** — 재확인 후 여백을 더 확보했습니다.

### Added

- **계층별 색상** — 플로우차트·클래스·상태 다이어그램의 노드가 **단계(계층)별로 서로 다른 색**으로 칠해져 구조가 한눈에 들어옵니다. (파랑→초록→노랑→보라→분홍…) 다크 모드에서도 글씨가 잘 보이도록 대비를 맞췄습니다.
- **글씨 크기 연동** — 상단의 −/＋ 폰트 크기 조절이 이제 **Mermaid 다이어그램 글씨에도 적용**됩니다.
- **파이 차트 색상** — 조각마다 뚜렷한 색과 범례가 나오도록 개선(이전엔 전부 흰색이었습니다).

### Changed

- 기본 글씨를 키우고, 곡선 엣지·간격을 다듬어 전반적으로 더 크고 읽기 좋게 렌더링합니다.

## [1.0.37] - 2026-08-04

### Fixed

- **🚨 Mermaid 다이어그램을 PDF로 내보내면 엉망으로 나오던 문제** — html2canvas가 인라인 SVG(특히 라벨을 그리는 `foreignObject`)를 캡처하지 못해 다이어그램이 **빈 박스로** 찍혔습니다. 이제 PDF 내보내기 직전에 각 다이어그램을 인쇄용 밝은 테마 + SVG 텍스트 라벨로 다시 그려 **PNG 래스터 이미지로 교체**한 뒤 캡처합니다. 다크 모드에서 내보내도 종이에 맞는 밝은 다이어그램으로, 한글 라벨까지 선명하게 나옵니다. (캡처 후에는 원래 SVG로 자동 복원)
- **다크 모드에서 다이어그램을 확대하면 색이 깨지던 문제** — 확대 오버레이의 배경이 흰색으로 고정되어 있어, 다크 테마로 렌더된 다이어그램이 흰 배경 위에 얹혀 "다크 모드가 풀린 것처럼" 보였습니다. 이제 오버레이 배경이 문서 테마를 따라갑니다(라이트/다크 각각).
- **다이어그램 아래쪽 글씨가 살짝 잘리던 문제** — viewBox에 여백을 더해 마지막 줄의 받침(g·j·p·q·y 같은 디센더)이 안 잘리도록 했습니다.

### Changed

- **Mermaid 다이어그램이 활성 컬러 테마를 따라갑니다** — 회색 일변도였던 기본 렌더링 대신, 13가지 테마(라이트/다크 포함)의 강조색·글자색·카드 배경에 맞춰 노드·화살표·라벨 색이 어우러지도록 바꿨습니다. 곡선 엣지·간격도 다듬어 더 깔끔해졌습니다.

## [1.0.36] - 2026-07-20

### Added

- **Mermaid 다이어그램 확대·축소·팬** — 복잡한 다이어그램이 작아서 안 보이던 문제 해결. 다이어그램에 마우스를 올리면 ⤢ 버튼이 뜨고, 누르거나(또는 다이어그램 클릭) 전체 화면으로 열립니다. 휠로 확대/축소(커서 기준), 드래그로 이동, 더블클릭으로 리셋, ESC/바깥 클릭으로 닫기. SVG 벡터라 아무리 키워도 선명합니다. 이미지 라이트박스와 같은 줌 엔진을 공유합니다.

## [1.0.35] - 2026-07-20

### Fixed

- **🚨 마크다운 파일을 두 개 이상 열면 창이 무한히 전환되던 치명적 버그 (v1.0.34에서 발생)** — v1.0.34의 "텍스트 탭 → 뷰어 재오픈" 안전망에 재진입 가드가 없어서, 재오픈이 만든 탭 변경 이벤트가 다시 자기 자신을 호출하고 `vscode.openWith`가 포커스를 가져가며 파일 사이를 끝없이 오갔습니다. 이제 파일당 한 번만 변환하고(탭이 닫힐 때까지 기록 유지), 이벤트 폭주는 디바운스로 합치며, 포커스를 뺏지 않도록(`preserveFocus`) 바꿔 루프가 구조적으로 불가능합니다.
